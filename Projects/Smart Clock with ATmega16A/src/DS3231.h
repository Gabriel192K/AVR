...
#define DS3231_TEMP_HIGH       (uint8_t)0x11
#define DS3231_TEMP_LOW        (uint8_t)0x12

// DS3231 register bits
// ******************************************************************************************************************
#define DS3231_STOP_CLOCK_BIT   (uint8_t)0x80
#define DS3231_TEMPERATURE_SIGN (uint8_t)0x80
...
{
    TWI_beginTransmission(DS3231_ADDRESS); // Begin transmission to DS3231
    TWI_write(DS3231_TEMP_HIGH);           // Start with temperature MSB register
    TWI_requestFrom(DS3231_ADDRESS);       // Request data from DS3231
    *temperature = TWI_read(TWI_NACK);     // Read temperature
    if (*temperature & DS3231_TEMPERATURE_SIGN) *temperature *= -1;
    TWI_endTransmission();
}
